"""
Postgres-backed state management for school pipeline runs.
...
Ported from Church Scraper db_state.py with school-specific field names
and connection retry fix for claim/complete operations.
"""
...
def _reset_conn() -> psycopg2.extensions.connection:
    """Force close and recreate the thread-local connection."""
    conn = getattr(_local, "conn", None)
    if conn is not None:
        try:
            conn.close()
...
QUEUE_WORKER_LOCK_ID = 839202  # Different from church scraper (839201) to avoid conflicts
...
def claim_county_task(scraper_type: str = "school") -> Optional[dict]:
    ...
    Uses _execute-style retry to handle broken connections (fixes idle worker bug).
    """
        ...
        try:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                if not row:
                    ...
                    return None
                cur.execute("""
                    UPDATE county_tasks SET status = 'processing', claimed_by = %s, claimed_at = %s
                    WHERE id = %s
...
def complete_county_task(task_id: int, result: dict, csv_data: Optional[str] = None,
    ...
    for attempt in range(2):
        try:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    UPDATE county_tasks SET status = 'done', completed_at = %s, result_json = %s
                    WHERE id = %s
...
def get_dispatch_progress(run_id: str) -> dict:
    ...
    completed = counts.get("done", 0) + counts.get("completed", 0) + counts.get("failed", 0)
    return {
        "total": total,
