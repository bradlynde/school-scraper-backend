        ...
        try:
            ...
            if not remaining_counties:
                ...
                with Pool(processes=MAX_WORKERS, maxtasksperchild=1) as pool:
                    # Pre-submit ALL counties to the pool immediately for true parallel processing
                    # This fixes the lazy task submission issue with imap_unordered()
                    ...
                    try:
                        # Collect results as they complete (out-of-order is handled)
                        while len(processed_counties) < len(async_results):
                            ...
                            for county, async_result in async_results.items():
                                ...
                                if async_result.ready():
                                    try:
                                        ...
                                        idx, county, result, processing_time = async_result.get(timeout=0.1)
                                        processed_counties.add(county)
                                        ...
                                        print(f"[{run_id}] Error getting result for {county}: {e}")
                                        import traceback
                                        traceback.print_exc()
                                        failed_counties.append(county)
                                        with checkpoint_lock:
                            ...
                            # Small sleep to avoid busy-waiting if no tasks are ready
                            if len(processed_counties) < len(async_results):
                                time.sleep(0.1)
